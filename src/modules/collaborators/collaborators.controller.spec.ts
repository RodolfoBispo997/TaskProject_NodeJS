...
describe("CollaboratorsController", () => {
  ...
  beforeEach(async () => {
    ...
    controller = await module.resolve<CollaboratorsController>(
  ...
  describe("findAll", () => {
    it("should return a paginated list of collaborators", async () => {
      ...
      jest
        .spyOn(service, "findAllByProject")
        .mockResolvedValue(mockedResponse as any);

      const response = await controller.findAllByProject(
        projectId,
        mockPaginationQuery,
      );
  ...
  describe("create", () => {
    ...
    it("should be able to handle validation errors", async () => {
      ...
      await expect(
        controller.create(projectId, { userId: "" }),
